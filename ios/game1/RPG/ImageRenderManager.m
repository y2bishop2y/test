...
// managers IVA for this image and passes back a pointer to the TexturedColorQuad
// structure within the IVA.
- (void)copyImageDetails:(ImageDetails*)aImageDetails;

// Method used to add a texture name to the list of textures that will be used to render
// the current contents of the render queue.
...
- (id)init 
{
	...
	if(self = [super init]) 
	{
		...
		// Init the indices array. This array will be used to specify the indexes into 
		// the interleaved vertex array. This array will allow us to just specify the specific 
		// interleaved array elements we want glDrawElements to render. We multiply by 6 
		// as we are using GL_TRIANGLE to render and we therefore define two triangles each
		// with three vertices to make a quad.
...
- (void)addImageDetailsToRenderQueue:(ImageDetails *)aImageDetails
{
	...
	// As we have added an images details to the render queue we need to increment
	// the iva index.
	ivaIndex++;
}
...
- (void)addTextureColoredQuadToRenderQueue:(TexturedColoredQuad *)aTCQ texture:(uint)aTexture
{
	...
	// As we have added a TextureColoredQuad tot he render queue we need to increment the iva index
	ivaIndex++;
}
...
- (void)renderImages
{
	...
	// Loop through the texture index array rendering the image as necessary
	for ( NSInteger textureIndex = 0; textureIndex < renderTextureCount; textureIndex++ ) {
		...
		// Init t he vertex counter. This will be used to identify how many elements need to 
		// be used within the indices array.
		int vertexCounter = 0;
		
		for( NSInteger imageIndex = 0; 
			imageIndex < imageCountForTexture[texturesToRender[textureIndex]];
			imageIndex++ ) {
			...
			// have inside the IVA for each image quad. Four vertices per quad so incrmement the vertex counter
		...
		// Now we have loaded the indicies array with indexes into the IVA, we draw those triangles
...
- (void) copyImageDetails:(ImageDetails*)aImageDetails 
{
	// Check to make sure that we have not exceeded the maxiumum size of the render queue. If the queie size
	// is exceeded then render the imge that are currently in the render managers queue.  This resets the
	...
	// Copy the images base texturedColoredQua into the assigned IVA index. This is necessary to make 
	// sure the texture and color information is loaded into the IVA. The Geometry from the image is loaded
	// when the image is transformed within the Image render method.
	memcpy( aImageDetails->texturedColoredQuadIVA, 
			aImageDetails->texturedColoredQuad,
