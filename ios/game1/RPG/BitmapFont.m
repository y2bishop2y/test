...
// the common height of the font which is used when calculating the postion of the font
...
// This method also associates an image with each character
...
- (id)initWithFontImageNamed:(NSString *)aFileName controlFile:(NSString *)aControlFile scale:(Scale2f)aScale filter:(GLenum)aFilter {
	...
	if ( self != nil ) { 
		...
		// Reference the font image which has been supplied and which contains the character 
		// bitmaps
...
- (id)initWithImage:(Image*)aImage controlFile:(NSString*)aControlFile scale:(Scale2f)aScale filter:(GLenum)aFilter {
	...
	if (self != nil ) {
		...
		// Initialize the array which is going to hold the bitmap fontchar structures
		charsArray = calloc(kMaxCharsInFont, sizeof(BitmapFontChar));
...
-(void) renderStringAt:(CGPoint)aPoint text:(NSString*)aText {
	...
	for (int i =0; i < [aText length]; i++ ) {
		...
		// Using the current x and y calculate the correct position of the character using the 
		// x and y offset for each character. This will cause the characters to all sit on the the
		// line correctly with tails below the line. The commonHeight which has been taken from 
		// the fonts control file is used within the calculation.
...
- (int)getWidthForString:(NSString*)string {
	...
	for ( int index = 0; index < [string length]; index++ ) {
		...
		// String the xAdvance value of the current character to stringWidth 
...
- (void)parseFont:(NSString *)controlFile {
	...
	// Create a holder for each lne we are going to work with
	NSString *line;
...
-(void)parseCommon:(NSString*)line {
	...
	// Setup the variables that are going to hold the information for the common line
	int scaleW;
	int scaleH;
	int pages;
