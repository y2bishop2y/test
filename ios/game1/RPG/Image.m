...
- (id) initWithImageNamed:(NSString *)aName filter:(GLenum)aFilter
{
	...
	if ( self != nil ) {
		...
		// Initalize the common properties for this image
		[ self initializeImage:	aName filter: aFilter ];
		
		// Set the width and height of the image to tbe the full width and height of the image
		// within the texture 
		imageSize	      = texture.contentSize;
		originalImageSize = imageSize;
		
		// Get the texture width and height which is to be used. For an image which is not using
		// a sub region of a texture then these values are the max width and height values from
...
- (id)initWithImageNamed:(NSString*)aName filter: (GLenum) aFilter subTexture:(CGRect)aSubTexture 
{
	...
	if ( self != nil ) 
	{
		// Save the sub texture rectangle
		subImageRectangle = aSubTexture;
		
		// Initialize the common properties for this image
		[self initializeImage:aName filter:aFilter];
		
		// Set the width and heigh of the image that has been passed into in.  This is defining a 
		// sub region within the larger textures.
		imageSize         = aSubTexture.size;
		originalImageSize = imageSize;
		
		// Calcuate the point within the texture from where the texture sub region will be started
		textureOffset.x = textureRatio.width  * aSubTexture.origin.x;
		textureOffset.y = textureRatio.height * aSubTexture.origin.y;
		
		// Calculates the width and height of the sub regiion this image is going to use.
		...
		// Init the image imageDetails structure
		[ self initializeImageDetails];
	}
	return self;
}
...
{
	// Create a new image which represents the deinfed sub image of this image
...
{
	// If the width or height passed in is < 0 or > 100 the log an error
	if ( aImageSize.width  < 0 || aImageSize.width  > 100
   	  || aImageSize.height < 0 || aImageSize.height > 100 ) 
	{
		MNLOG( @"ERROR - Image: Illegal % provide to setImageSizeToRender 'width=%f, height=%f'", aImageSize.width, aImageSize.height );
		
		return;
	}
	
	// Using the original size of this image, calculate the new image width baed on the 
	// percentage provided
	...
	textureSize.width  = (textureSize.width  * imageSize.width  ) + textureOffset.x;
	textureSize.height = (textureSize.height * imageSize.height ) + textureOffset.y;
	
	// Initialzie the image details. This will recalculate the images geometry and texture
	// coordinate in the imageDetails structure.
	[self initializeImageDetails];
...
- (void)renderCentered {
	...
	// Take a copy of the images point as we will be adjusting this so that the iamge 
	// is rendered centered on that point
	CGPoint pointCopy = point;
	
	// Adjust the point the image is going to be rendered at so that the center of the 
	// image is rendered at that point.
	point.x = point.x - ((imageSize.width  * scale.x) / 2 );
	point.y = point.y - ((imageSize.height * scale.y) / 2 );
	
	// Mark the image as dirsty and render the image
	dirty = YES;
...
- (void) render
{
	...
	// Add this image to the render queue.  This will cause this image to be rendered the next 
	// time. The RenderManager is asked to render. It also copes the data over to the image renderer
	// IVA. It is this data that is changed nest by applying the image matrix
	...
	if ( dirty ) {
		...
		// Load the identity matrix before applying transforming the matrix for this usage. The 
		// order in which the transformations are applied is important.
		loadIdentityMatrix(matrix);
		
		// Translate the postion of the image first
		translateMatrix(matrix, point);
		
		// If this imsage has been config t obe flipped vertically or horizontally 
		// then set the scale for the image to -1 for the appropriate axis and then translate
		// the image so that the image origin is rendered in the correct place
		if ( flipVertically) {
		...
		// No point in calculating a rotation matrix fi there is no rotation been set
		if ( rotation != 0) {
			rotateMatrix(matrix, rotationPoint, rotation);
		}
		
		// No point in calculating scale if no scale has been set.
...
{
	// Grab a reference to the texture image rendered and shred game singletons
	textureManager           = [TextureManager sharedTextureManager];
	sharedImageRenderManager = [ImageRenderManager sharedImageRenderManager];
	sharedGameController	 = [GameController sharedGameController];
	
	// Set the image name to the name of the image file used to create the textiure. This is also
	// the key within the texture manager for grabbing a texture form the cache.
	...
	// Set the texture name to the OpenGL texture name give to the Texture2D object
	textureName = texture.name;
	
	// Get the full widht and height of the texture.  We could keep accessing the getter of the texture
	// instance but keeping our own copy cuts down a little n the messaging
	fullTextureSize.width  = texture.width;
	fullTextureSize.height = texture.height;
	
	// Grab the texture width and height ration from the texture
...
- (void)initializeImageDetails
{
	...
	// Set up a TexturedColorQuad structure which is going to hold the original information
	// about our image. This structure will never change, but will be used when performing
	// transforms on the image with the results being loaded into the RenderManager using
	// this images render index
	if ( !imageDetails) {
	...
	// Setup the geometry for the image
