...
{
		// Try to get a texture from cachedTextures with supplied key.
	...
	// We are using imageWithContentsOfFile rather that imageNamed, as imageNamed caches the image in the device.
	...
	// Return the texture which is autorelased as the caller is responsible for it.
...
- (BOOL) releaseTextureWithName:(NSString *)aName
{
	...
	// No texture wasfound with the supplied key so log that and return NO
