...
- (void)dealloc
{
	MNLOG(@"INFO - Texture2D: Deallocating");
...
- (id) initWithImage:(UIImage *)aImage filter:(GLenum)aFilter
{
	...
	if ( self != nil ) {
		...
		// raise an error. We don't want the application to continue as a missing image could create 
		// unexpected results
		NSAssert( image, @"ERROR - Texture2D: The supplied UImage was null.");
		
		// Check to see if the image contains alpha information by reading the alpha into from the image
		...
		// Check to see what pixel format the iamge is using
		...
		// We need to make sure that the texture we create is a power of 2 so start at 1 and the multiply
		// i by 2 until i is greater than the width -1 of the image. This will give us our power 2 width
		// and will be set as the textures width.
		
		// Hold the poewr of 2 value being calculated
		NSUInteger pot;
		...
		// we need to make sure that the texture is now not bigger than 1024 x 1024 whci is the largest
		// single texture size the iPhone can handle. if it is too big then the image is scalled down 
		// by 50%
		...
		// We now need to move the origin with the context we hve created. We want to render the image into
		// the CG context so that the bottom of the rendered image will be at the bottom of the newly created
		// context. To do this we move the Y element of the origin (which is the top left corner) down by
		// the difference between the image height and the context height
		CGContextTranslateCTM(context, 0, height -contentSize.height);
		
		// If transform is something other thant the indentity matrix then apply that transform to the 
		// context. This is normally set due to the texture size being greater thatn the max allowed
		...
		// Now we are done with the setup we can render the image which was passed into the new context
		...
		// Configure the textrures min and mag filters. This MUST hapepn for textures to show up on the iPhone.
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, aFilter);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, aFilter);
		
		// Baesd on the pixel format of the image use glTexImage2D to load the data from the CG Context
		...
		// We need to calcuate the max texture coord for our image within the texture.
		// As the texture size is power of 2 and could betherefore larger thant the actual image
		// which it contains, we need to calcuate the maximum s, t values using the size of the 
		// content and the size of the texture.
		maxS = contentSize.width  / (float)width;
		maxT = contentSize.height / (float)height;
		
		// So that we can convert pixels into texture coords easily we need to calculate
		// the pixel to texture ratio for this texture. Remember that the max s, t texture
		// coords you can have are 1.0, 1.0f. Behavior if the texture coors are 
		// greater than 1.0f will be based on the clamping and wrapping configuration fo this
		// texture.
		textureRatio.width  = 1.0f / (float)width;
		textureRatio.height = 1.0f / (float)height;
		
		// We are now done iwith the CG context so we can release it and the memory we allocated to
		// store the data within the context.
		CGContextRelease(context);
		free(data);
	}
	// Returns self with autorelease.  The receiver is responsible for retaining this instance
	return self;
}
