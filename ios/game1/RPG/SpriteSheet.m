...
// Using the image privded, cache the sprite information for each sprite in the sprite sheet
- (void)cacheSprites;
								  ...
								  spacing:(NSUInteger)aSpacing margin:(NSUInteger)aMargin imageFilter:(GLenum)aFilter
{
	...
	// As we hve not found a sprite sheet we need to create a new one
...
+(SpriteSheet*)spriteSheetForImage:(Image *)aImage sheetKey:(NSString *)aSheetKey spriteSize:(CGSize)aSpriteSize 
						   spacing:(int)aSpacing marging:(NSUInteger)aMargin
{
	// Create a variable that will hold the sprite sheet to return.
	SpriteSheet* cachedSpriteSheet;
...
{
	// Check to make sure that the requeseted coordinates are within the bounds of the 
	// spriteSheet
	if ( aPoint.x > horizSpriteCount - 1 || aPoint.y < 0 
	  || aPoint.y > vertSpriteCount  - 1 || aPoint.y < 0 ) {
		return nil;
	}
...
- (void)cacheSprites
{
	// Calculate how many sprites there are horizontally and vertically given their size eand margin
	...
	// Intialize the structure to hold the geometry, texture and colour vertices
