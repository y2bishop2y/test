...
- (id)init {
	...
	if(self = [super init]) {
		...
		// Initialie the array that will store the animation frames
...
- (void)addFrameWithImage:(Image *)aImage delay:(float)aDelay {
	...
	// If we try to add more frames that we have storage for then increase the storage
...
- (void)updateWithDelta:(float)aDelta {
	...
	// If the displayTime has exceede the current frames delay then switch frames
	if (displayTime > frames[currentFrame].delay ) {
		
		currentFrame += direction;
		
		// Rathe than set displayTime back to 0, we set it to the difference between 
		// display time and frames delay.  This will cause frames to be skipped as 
		// necessary if the game should bog down
		displayTime -= frames[currentFrame].delay;
		
		// If we have reached the end or start of the animation, decide on what needs
		// to be done based on the animations type
		if ( type = kAnimationType_PingPong 
			&& (currentFrame == 0 || currentFrame == frameCount - 1 || currentFrame == bounceFrame ) ) {
