...
@interface  PackedSpirteSheet (Private)
...
@implementation PackedSpirteSheet
...
+ (PackedSpirteSheet*)packedSpriteSheetForImagenamed:(NSString *)aImageName controlFile:(NSString *)aControlFile imageFilter:(GLenum)aFilter
{
	// Set up a holder for our PackedSpriteSheet
	PackedSpirteSheet *cachedPackedSpriteSheet;
	
	// IF the cachedSpriteSheets dictionary does not already exist then create one
	if ( !cachedPackedSpriteSheets ) {
		
		cachedPackedSpriteSheets = [[NSMutableDictionary alloc] init];
	}
	// Try to find a cached packed sprite sheet that has the same image name as the one provided. If one 
	// is found then reference to that is returned.
	if ( cachedPackedSpriteSheet = [cachedPackedSpriteSheets objectForKey:aImageName ] ) {
		
		return cachedPackedSpriteSheet;
	}
	// If no match is found then we create a new PackedSpriteSheet, add ti to the cache and return
	// a reference
	cachedPackedSpriteSheet = [[PackedSpirteSheet alloc] initWithImageNamed:aImageName controlFile:aControlFile filter:aFilter];
...
{
	PackedSpirteSheet *cachedPackedSpriteSheet = [cachedPackedSpriteSheets objectForKey:aKey ];
	...
	if ( cachedPackedSpriteSheet ) {
		...
		MNLOG( @"WARNING = PackedSpriteSheet: Key '%@' could not be found to release.", aKey );
	}
	return NO;
}
...
- (id)initWithImageNamed:(NSString *)aImageFileName controlFile:(NSString *)aControlFile filter:(GLenum)aFilter
{
	if ( self = [super init]) 
	{
		...
		// Creates a dictionary from the plist control file which shold be in the main bundle. The 
		// control files extensions should awlays be plist
		controlFile = [[NSDictionary alloc] initWithContentsOfFile:[[NSBundle mainBundle]
...
{
	// Return the image for the key provied form the sprites dictionary
	...
	MNLOG( @"ERROR - PackedSpriteSheet: Sprite could not be found for key '%@' ", aKey );
...
@implementation PackedSpirteSheet (Private)

- (void)parseControlFile:(NSDictionary *)aControlFile
{
	// Create a dictionary from the eintries in the frames dictionary in the plist file
	NSDictionary *framesDictionary = [controlFile objectForKey:@"frames" ];
	
	// Loop through all the keys in the framesDictionary and process its dictrionary
	for ( NSString *frameDictionaryKey in framesDictionary) 
	{
		...
		// Add a reference to the sprite to the sprites dictionary for us to grab later
		// The returned image is autoreleased but as the retain count is increased when we add the image
		// reference to the dictionary, we don't need to do an extra retain
